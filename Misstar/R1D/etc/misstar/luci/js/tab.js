...
layui.config({
	...
	Tab.prototype.tabAdd = function(data) {
		...
		if(tabIndex === -1) {
			globalTabIdIndex++;
			var content = '<iframe src="' + data.href + '" data-id="' + globalTabIdIndex + '" id="' + data.href +'" height="100%"></iframe>';
			var title = '';
			...
			if(that.config.closed) {
				title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="' + globalTabIdIndex + '"data-type="' + data.href + '">&#x1006;</i>';
			}
			//添加tab
...
//
//				//$(this).height(1000);//$(this).context.contentDocument.body.scrollHeight;
//
//			});
			if(that.config.closed) {
				//监听关闭事件
				ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
					that.config.beforeClose($(this).attr('data-type'));
