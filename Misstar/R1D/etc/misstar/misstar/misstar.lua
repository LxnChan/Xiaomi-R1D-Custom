...
function mis_update()
    local result = {}
    local code=LuciUtil.exec("wget http://cloud.lifeheart.cn:188/miwifi/MT/tools/appstore/update.sh -O /tmp/update.sh >/dev/null 2>/dev/null && chmod +x /tmp/update.sh >/dev/null 2>/dev/null && /tmp/update.sh >/dev/null 2>/dev/null")
    result["code"] = code
    result["list"] = list
    if code ~= 0 then
